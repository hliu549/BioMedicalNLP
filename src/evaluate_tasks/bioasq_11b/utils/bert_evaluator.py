...
class BertEvaluator(object):
    def __init__(
        self, model, processor, tokenizer, args, split="dev", dump_predictions=False, inference_only=False
    ):
        self.args = args
        self.model = model
    ...
    def get_scores(self, silent=False):
        ...
        predicted_labels, target_labels, logits_list = list(), list(), list()
        ...
        for idx, example_id in enumerate(self.examples_ids):
            logits_dict[example_id] = logits_list[idx]
        ...
        if self.inference_only:
            return (result, predicted_labels_dict, logits_dict,)
        return (
            result,
            predicted_labels_dict,
        )
